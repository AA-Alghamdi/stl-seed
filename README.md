...
- **Hierarchical Bayes posterior over the methodology gap** ([`paper/hierarchical_bayes.md`](paper/hierarchical_bayes.md)). NumPyro NUTS over the 120-cell quant-size sweep with partial pooling across (model, task). Posterior mean of `delta_global` = $+82.82$ ρ-units, 95% credible interval $\[+35.22, +129.44\]$, $P(\\delta\_\\text{global} > 0) = 0.9995$. 0 divergences, $\\hat{R} \\le 1.001$, ESS $\\ge 2178$. The (model, task) population-mean methodology gap is credibly positive across the 5×4 grid.
- **Information-theoretic verifier-noise analysis** ([`paper/information_theory.md`](paper/information_theory.md)). $I(\\text{STL ρ}; \\text{success}) = 0.94$ bits on repressilator, 0.88 on glucose-insulin; PAV-v2 at the upper bound is $0.92$ and $0.87$ respectively. STL verifier-fidelity noise is $\\sim 10^{-15}$ vs PAV's $\\sim 10^{-1}$ (16+ orders of magnitude on the dynamic-range-normalized scale).
- **Memory-quality Pareto** ([`paper/memory_pareto.md`](paper/memory_pareto.md)). Connects the artifact to QLoRA / bitsandbytes lineage. Sweet spot: `qwen3-1.7b-4bit` at $3.27$ GB peak, smallest configuration that preserves the methodology gap on 4/4 tasks while exhibiting the SERA-saturation transition on toggle. `qwen3-1.7b-bf16` at 5.53 GB is strictly dominated.
- **STLCG++ formal comparison** ([`paper/stlcgpp_comparison.md`](paper/stlcgpp_comparison.md)). Two novel contributions over STLCG / STLCG++ (Leung 2020, Hashemi 2025): (1) straight-through-style bridge from a discrete LLM vocabulary to the STLCG++ continuous-control gradient via expected-action $\\bar{u}\_t = \\sum_k p\_{t,k} V_k$ plus a bias term that drops the $p\_{t,k}$ Jacobian factor à la Bengio 2013; (2) per-step inference-time MPC application that takes the gradient on a partial-then-extrapolated control at every decoding step. On the differentiable cell our $\\nabla \\rho$ is bit-identical to STLCG++.
- **Inference-time compute scaling laws** ([`paper/scaling_laws.md`](paper/scaling_laws.md)). Task-structure-dependent: glucose-insulin admits $\\bar{ρ}(t) = -8.35 \\cdot t^{-0.241} + 27.2$ (R² = 0.81, sub-linear approach to a +27 ceiling, sign-consistent with Snell et al. 2024). Other four tasks are step-function geometry, not power-law.
- **PAV V2 strengthened baseline** ([`paper/pav_v2.md`](paper/pav_v2.md)). With model selection (4×4 grid) + Setlur on-policy MC labels, glucose-insulin gap STL−PAV tightens to $-0.038$ (was $-0.181$ with the v1 offline-kNN strawman); on bio_ode.repressilator PAV-v2 now matches STL at AUC = 1.000.
- **FMAI 2026 workshop draft** ([`paper/fmai_2026_draft.md`](paper/fmai_2026_draft.md)). 4-page skeleton ready for ICML 2026 FMAI submission (deadline 2026-05-08). Title: *"When Soft Beats Hard, and When It Doesn't."*
- **Money plot** ([`paper/figures/money_plot.png`](paper/figures/money_plot.png)). Single conference-quality figure capturing the 5×4 methodology-gap heatmap with the SERA-saturation transition flagged on toggle at 1.7B.
- **Coherent narrative** ([`STORY.md`](STORY.md)). The pitch document tying everything together.
- **3-size scaling extension** ([`paper/quant_size_4b_addendum.md`](paper/quant_size_4b_addendum.md)). Qwen3-4B-4bit fires `METHODOLOGY MATTERS` too. 6 (model, precision) configurations spanning 3 sizes (0.6B, 1.7B, 4B) × 3 precisions (bf16, 8-bit, 4-bit). Toggle saturation reproduces deterministically across (1.7B-bf16, 1.7B-4bit, 4B-4bit); other 3 tasks stay solidly methodology-mattering at all 6 cells.
- **Failure case study (gradient-guided on repressilator)** ([`paper/failure_case_study_repressilator.md`](paper/failure_case_study_repressilator.md)). Diagnostic deep dive numerically verifying the regime-II conditions. The sharp finding: at $t=0$ the gradient cosine against the satisfying-corner direction is $-0.85$. The cliff is not just flat; it is adversarially oriented along the early decoding axes.
- **Projection theorem** ([`paper/projection_theorem.md`](paper/projection_theorem.md)). Formal proof that gradient-guided in the (flat-prior, $\\lambda \\to \\infty$, $T \\to 0^+$) limit equals projected gradient ascent on $J$ restricted to $\\text{conv}(V)$. Theorem 2 gives the biased-projection form for non-uniform LLM priors.
- **Snell scaling positioning** ([`paper/snell_scaling_position.md`](paper/snell_scaling_position.md)). Places our scaling regime against Snell et al. 2024 (test-time-compute scaling for math reasoning). The regime claim (sub-linear, verifier-guided diminishing returns, cross-over ratio comparable, breaks at narrow-attractor tasks) is defensible; same-exponent claim is not, because Snell does not publish a power-law exponent.
- **Aksaray STL-as-RL-reward comparison** ([`paper/aksaray_comparison.md`](paper/aksaray_comparison.md)). Our per-step bias is the first-order Taylor approximation of the per-step Q-increment Aksaray's STL-rewarded Q-learning converges to. Computed by autodiff through a differentiable simulator at inference time rather than estimated from rollouts at training time.
- **Pre-registration ledger** ([`paper/preregistration_ledger.md`](paper/preregistration_ledger.md)). 11 pre-registered claims; 7 fired correctly, 0 surprising in registered direction. PR2 (quantization-survival) and PR4 (gradient-guided cross-task negative) were the genuinely risky pre-regs.
- **Falsification criteria** ([`paper/falsification_criteria.md`](paper/falsification_criteria.md)). 8 major claims, each with a specific falsifying experiment + cost estimate. Mathematical / definitional claims explicitly separated from empirical ones.
- **Results dashboard** ([`paper/RESULTS.md`](paper/RESULTS.md)). Single-page navigation index of every doc and script in the artifact, organized by reading time.
