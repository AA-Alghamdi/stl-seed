...
- **417 unit tests pass + 6 platform-skipped + 1 xfailed** across 27 test modules in `tests/` on the smoke-test machine. Coverage is **91%** on `src/stl_seed/`. The xfailed test is `test_time_shift_invariance` — the property holds for continuous-time STL but fails on the discrete-grid evaluator when Δ isn't a multiple of grid spacing (a discretization artifact, not an evaluator bug; documented in the test). Tier-3 to Tier-8 autonomous-loop additions: `test_inference.py` (gradient-guided sampler), `test_auto_tune.py` (spec auto-tuning), `test_adversary.py` (Goodhart adversary), `test_pav_baseline.py` (PAV vs STL), `test_stl_properties.py` (13 algebraic invariants), `test_phase2_validation.py` (mock-backend dry-run).
