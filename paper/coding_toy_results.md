...
- `src/stl_seed/specs/coding_specs.py`. registers `coding.fix.easy` with formula `F_[0, 6] (test_pass_rate > 0.5)`. The spec uses the same `Eventually + Predicate` form as `bio_ode` specs and respects the conjunction-only conjunction-only convention.
