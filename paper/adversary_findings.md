...
Per `paper/theory.md` §6, the spec-completeness term $R_\text{gold}(\tau) - R_\text{spec}(\tau)$ is the Goodhart-relevant residual once the verifier-fidelity term is collapsed by the choice of formal STL robustness as the verifier. The adversary above provides a constructive lower bound on $\sup_\tau [R_\text{spec}(\tau) - R_\text{gold}(\tau)]$ restricted to the spec-satisfying half-space. A negative gap (adv gold < mean satisfying random gold) means the spec is *locally* exploitable: there exist high-$\rho$ trajectories that score worse under the unstated gold than a random spec-satisfying baseline.
