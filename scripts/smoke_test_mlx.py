"""MLX QLoRA smoke test on Qwen3-0.6B.
...
Pipeline:
  1. Load filtered glucose-insulin trajectory IDs/weights from
     ...
     `scripts/filter_pilot.py`). Rejoin with the source
...
# Spec for the held-out trajectories. Same one used to score the pilot data.
...
def _write_report(
    *,
    ...
    lines: list[str] = []
    lines.append("# MLX QLoRA smoke test report")
    lines.append("")
    ...
    lines.append(f"| seed | {_SEED} | dataset consistency |")
    lines.append("| optimizer | AdamW | mlx.optimizers default for LoRA |")
    lines.append("")
    lines.append("## Dataset")
    lines.append("")
    lines.append(
        f"- **Source:** `{_FILTERED_PARQUET.relative_to(_REPO_ROOT)}` (built by `scripts/filter_pilot.py`)."
    )
...
def main() -> int:
    ...
    # Backend followups (always recorded, regardless of pass/fail).
    followups.append(
    ...
    followups.append(
        "RunPod training uses the bnb backend, not MLX, so the MLX wrapper "
        "bug above does not block the CUDA path. Validate the bnb path with "
        "the same dataset shape produced here."
    )
    followups.append(
