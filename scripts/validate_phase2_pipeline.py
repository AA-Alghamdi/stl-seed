...
# --------------------------------------------------------------------------
# Stage 4: public smoke checks.
# --------------------------------------------------------------------------


def stage_public_smoke_checks() -> StageResult:
    """Stage 4 is a SKIP in the public repo.

    Returning SKIP rather than removing the stage keeps stage indexing stable
    for the downstream report writer and lets a future operator drop in their
    own optional smoke-check command if they fork.
    """
    return StageResult(
        name="stage_public_smoke_checks",
        status="SKIP",
        duration_s=0.0,
        detail="optional smoke checks not configured; SKIP",
        findings=[],
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    _print_banner("Stage 4: optional smoke checks")
    results.append(stage_public_smoke_checks())
    _print_kv("status", results[-1].status)
    _print_kv("duration", f"{results[-1].duration_s:.2f}s")
    for f in results[-1].findings:
        print(f"    FINDING: {f}")
