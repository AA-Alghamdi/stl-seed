"""Generate the local-validation pilot trajectory store.
...
deliberately skipped because it requires the optional `mlx` extra and is
covered by the backend smoke tests.

This script also runs the runner-vs-canonical STL evaluator consistency
check: the runner ships an inline NumPy reference evaluator, and the
canonical implementation lives in `stl_seed.stl.evaluator`. We sample
...
def main() -> int:
    console.rule("[bold]Generate pilot trajectory store")
