"""Power analysis with empirical ICC from the pilot trajectory store.
...
def _write_paper(
    ...
    body = dedent(
        ...
        *Date: 2026-04-24. Companion to paper/theory.md §5.*
        ...
        (ICC) of robustness rho estimated from the pilot dataset.
        The original analysis used a worst-case plug-in
...
{n_seeds_block}
        ...
        2. The pilot contains two policy classes (random + heuristic)
           ...
           the canonical sweep will use. The LLM-policy leg is run separately
           when an MLX-capable Qwen3 build is available locally; per FM4 in theory.md §7, MLX vs bnb numerical
...
def main() -> int:
    console.rule("[bold]Empirical-ICC power analysis")
