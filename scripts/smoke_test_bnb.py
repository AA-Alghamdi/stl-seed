"""bitsandbytes 4-bit QLoRA smoke test on Qwen3-0.6B (RunPod-only).
...
# Hyperparameters. chosen to mirror the MLX smoke test so the two
...
def _write_report(
    *,
    ...
    lines: list[str] = []
    lines.append("# bnb QLoRA smoke test report")
    lines.append("")
    ...
    lines.append(f"| learning_rate | {_LR} | smoke-test override (matches MLX smoke test) |")
    ...
    lines.append(
        ...
        "(built by `scripts/filter_pilot.py`)."
    )
...
def main() -> int:
    console.print(
        Panel.fit(
            "bnb 4-bit QLoRA smoke test on Qwen3-0.6B-Instruct\n"
            ...
            title="[bold]bnb backend smoke test",
        )
    )
