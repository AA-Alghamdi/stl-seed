...
when the `training` subpackage is available; if it is not, this module ships
a self-contained reference formatter that produces the same JSON-schema
output. The fallback formatter is documented and tested here so we can swap
...
def _format_trajectory_as_text(
    ...
    requested horizon / action_dim. Pinning this schema here keeps
    `filter.dataset` self-contained.
    """
