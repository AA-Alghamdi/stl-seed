...
These specifications describe the *form* of STL formulae that ``stl-seed``
uses to score control trajectories. A concrete robustness evaluator can
target either ``stljax`` for autograd, or a NumPy reference implementation.
...
* Every numerical threshold is sourced from biological / clinical literature
  and cited inline at the point of use.
...
class Negation:
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.inner, Predicate):
            raise TypeError("Negation may only wrap a Predicate.")
...
class STLSpec:
    ...
    formula_text : str
        Standard STL syntax for the formula, for inclusion in the paper.
    """
