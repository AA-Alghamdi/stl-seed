...
* **Action diversity**: per-prompt summary of how distinct the model's
  generations are. Added after a smoke-test diagnostic produced an identical
  first action, indicating memorization on the dominant training pattern.
  This metric makes the failure mode falsifiable in eval.
"""
...
def action_diversity(
    ...
    * ``first_action_uniqueness``. fraction of unique first actions
      ...
      the same first action (the memorization failure mode).
